...
function StatCard({ item, compactValue = false }) {
  return (
    <article className="group relative min-h-[126px] min-w-0 overflow-hidden rounded-2xl border border-slate-200/90 bg-white px-3 py-4 shadow-[0_10px_24px_-19px_rgba(15,23,42,0.5)] transition-all duration-200 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-[0_18px_32px_-22px_rgba(15,23,42,0.55)] sm:min-h-[152px] sm:rounded-[24px] sm:px-6 sm:py-7 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-slate-700">
      <div className="flex min-w-0 flex-col items-start gap-2 sm:flex-row sm:justify-between sm:gap-4">
        <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-base text-white shadow-[0_8px_16px_-8px_rgba(15,23,42,0.7)] transition-transform duration-200 group-hover:scale-105 sm:h-12 sm:w-12 sm:rounded-[15px] sm:text-xl ${item.iconClass}`}>
          {item.icon}
        </span>
        <strong className={`min-w-0 break-words text-left font-extrabold leading-tight tracking-tight text-slate-900 tabular-nums sm:text-right dark:text-white ${compactValue ? "text-lg sm:text-[30px]" : "text-2xl sm:text-4xl"}`}>
          {item.value}
        </strong>
      </div>
      <p className="mt-3 text-[10px] font-extrabold uppercase tracking-wide text-slate-600 sm:mt-6 sm:text-sm dark:text-slate-300">
        {item.title}
      </p>
