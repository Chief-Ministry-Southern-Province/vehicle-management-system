...
import { createDriver, getDrivers, updateDriver } from "../../api/authApi";
...
function DriverForm({ driver, existingIds, onCancel, onSave }) {
  ...
  const field = "mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-50";
  ...
  return (
    <form onSubmit={submit} className="rounded-2xl border border-blue-100 bg-blue-50/40 p-5">
      <div className="flex items-center justify-between">
        <div><h2 className="text-lg font-bold text-slate-900">{editing ? "Edit Driver" : "Register Driver"}</h2><p className="text-sm text-slate-500">All fields are managed by the subject officer.</p></div>
        <button type="button" onClick={onCancel} className="rounded-lg p-2 text-slate-400 hover:bg-white hover:text-slate-700" aria-label="Close form"><FiX /></button>
      </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {fields.map(([name, label, type]) => (
          <label key={name} className="text-sm font-semibold text-slate-700">
            {label}
            <input name={name} type={type} value={form[name]} onChange={change} disabled={editing && name === "id"} required className={`${field} disabled:bg-slate-100 disabled:text-slate-500`} />
          </label>
        ))}
      ...
      <div className="mt-5 flex justify-end gap-3">
        ...
        <button className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700"><FiSave /> Save Driver</button>
      </div>
    </form>
  );
}
...
export default function DriverDirectory() {
  ...
  const filteredDrivers = useMemo(() => {
    ...
    return drivers.filter((driver) => [driver.id, driver.fullName, driver.nic, driver.licenceNumber, driver.vehicle, driver.status]
  ...
  const saveDriver = async (driver) => {
    ...
    try {
      const response = exists
        ? await updateDriver(driver.id, toDriverPayload(driver))
        : await createDriver(toDriverPayload(driver));
      const saved = normalizeDriver(response?.data?.driver);
      setDrivers((current) => exists
        ? current.map((item) => item.id === driver.id ? saved : item)
        : [saved, ...current]);
    ...
    <DashboardLayout>
      <main className="min-h-screen space-y-6 bg-slate-50 p-6">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div><p className="text-sm font-semibold text-blue-600">Fleet Management</p><h1 className="mt-1 text-2xl font-bold text-slate-900">Driver Directory</h1><p className="mt-1 text-sm text-slate-500">Register drivers and maintain their identity, licence, allocation, and status details.</p></div>
          <button onClick={() => setEditingDriver({ ...EMPTY_DRIVER })} className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700"><FiPlus /> Add Driver</button>
        </header>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1550px]">
              ...
              <tbody>{!loading && filteredDrivers.map((driver) => <tr key={driver.id} className="border-t border-slate-100 hover:bg-slate-50">
                <td className="px-4 py-4 font-semibold text-blue-600">{driver.id}</td><td className="px-4 py-4 font-semibold text-slate-900">{driver.fullName}</td><td className="px-4 py-4 text-slate-600">{driver.dateOfBirth}</td><td className="px-4 py-4 text-slate-600">{driver.nic}</td><td className="max-w-60 px-4 py-4 text-slate-600">{driver.address}</td><td className="px-4 py-4 text-slate-600">{driver.contactNumber}</td><td className="px-4 py-4 text-slate-600">{driver.bloodGroup}</td><td className="px-4 py-4 text-slate-600">{driver.licenceNumber}</td><td className="px-4 py-4 text-slate-600">{driver.licenceType}</td><td className="px-4 py-4 text-slate-600">{driver.licenceRenewalDate}</td><td className="px-4 py-4 font-medium text-slate-800">{driver.vehicle}</td><td className="px-4 py-4"><StatusPill status={driver.status} /></td><td className="px-4 py-4"><button onClick={() => setEditingDriver({ ...driver })} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-600 hover:border-blue-200 hover:text-blue-600"><FiEdit2 /> Edit</button></td>
              </tr>)}</tbody>
            </table>
          </div>
