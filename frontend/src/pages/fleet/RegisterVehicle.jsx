...
export default function RegisterVehicle() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      navigate(
        `/vehicledetails/${encodeURIComponent(response.data.vehicle.registration_number)}`,
      );
    } catch (error) {
      ...
      toast.error(
        errors
          ...
          : error?.message || "Unable to register vehicle.",
      );
    } finally {
      setSaving(false);
    }
        ...
        <header className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Add a vehicle and its operational details to the fleet registry.
            </p>
          </div>
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="inline-flex items-center justify-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-semibold text-slate-700"
          >
            <FiArrowLeft />
            Cancel
          </button>
        ...
        <div className="grid gap-5 xl:grid-cols-3">
          <div className="space-y-5 xl:col-span-2">
            <section className="rounded-2xl border bg-white p-5 shadow-sm">
              <h2 className="text-lg font-bold">Basic identification</h2>
            ...
            <section className="rounded-2xl border bg-white p-5 shadow-sm">
              <h2 className="text-lg font-bold">Technical and compliance</h2>
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                <label>
                  VIN
                  <input
          ...
          <aside className="space-y-5">
            <section className="rounded-2xl border bg-white p-5 shadow-sm">
              ...
              <button
                ...
                className="mt-4 flex w-full flex-col items-center rounded-xl border-2 border-dashed border-blue-200 bg-blue-50 p-6 text-blue-700"
              >
                {preview ? (
                  <img
                    src={preview}
