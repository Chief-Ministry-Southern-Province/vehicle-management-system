...
export default function RequestHistory() {
  ...
  return <DashboardLayout><div className="space-y-6"><div><h1 className="text-3xl font-bold">Request History</h1><p className="mt-1 text-gray-500">View all your vehicle requests, including approved requests.</p></div>
    ...
    <div className="overflow-hidden rounded-xl border bg-white shadow-sm"><div className="overflow-x-auto"><table className="w-full"><thead className="border-b bg-gray-50"><tr className="text-left text-sm text-gray-600"><th className="p-4">Request ID</th><th className="p-4">Date</th><th className="p-4">Destination</th><th className="p-4">Purpose</th><th className="p-4">Pax</th><th className="p-4">Priority</th><th className="p-4">Status</th><th className="p-4">Actions</th></tr></thead><tbody>
      {visibleRequests.map((request) => <tr key={request.id} className="border-b hover:bg-gray-50"><td className="p-4 font-medium text-blue-600">REQ-{String(request.id).padStart(4, "0")}</td><td className="p-4 text-gray-600">{new Date(request.created_at).toLocaleDateString()}</td><td className="p-4">{request.destination}</td><td className="p-4 text-gray-600">{request.purpose}</td><td className="p-4 text-center">{request.passenger_count}</td><td className="p-4 text-xs font-semibold uppercase text-gray-600">â— {request.department_priority || "Not set"}</td><td className="p-4"><span className={`rounded-full px-3 py-1 text-xs font-medium capitalize ${statusColor[request.status] || "bg-gray-100 text-gray-700"}`}>{request.status?.replaceAll("_", " ")}</span></td><td className="p-4"><button type="button" onClick={() => navigate(`/employee/requests/${request.id}`)} className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-blue-700" aria-label={`View request REQ-${String(request.id).padStart(4, "0")}`}><FiEye />View</button></td></tr>)}
