  ...
  useEffect(() => {
    const loadRequests = async () => {
      try {
        ...
        setStats(response?.data?.stats || { pending: 0, allocated: 0 });
      } catch (requestError) {
    ...
    <DashboardLayout>
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-3 rounded-2xl border bg-white p-3 shadow-sm">
          ...
          <button type="button" onClick={() => setView("allocated")} className={`rounded-xl px-5 py-2.5 text-sm font-semibold ${view === "allocated" ? "bg-emerald-600 text-white" : "bg-slate-50 text-slate-600"}`}>Allocated ({stats.allocated})</button>
        </div>
