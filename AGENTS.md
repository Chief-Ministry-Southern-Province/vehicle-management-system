...
- Vehicle records include identity/specification, capacity, fuel configuration and efficiency, compliance expiries, insurance, assignment, status, images, and JSON-backed service/repair/fuel history. Subject officers can add or delete persisted vehicle images from the vehicle details editor; deletion is applied when the vehicle update is saved and removes both the database reference and stored public image file. New registrations capture revenue-licence, insurance, and emission expiry dates; `registration_expiry` remains available for existing records but is not shown in Register Vehicle. Register Vehicle offers the standard vehicle types plus an **Add a new vehicle type** choice; its custom entry is submitted and persisted as the canonical `vehicle_type` (nonempty string up to 100 characters).
