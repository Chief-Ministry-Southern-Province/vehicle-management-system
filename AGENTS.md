...
- The deputy secretary's Pending Allocation Queue additionally displays each request's abbreviated saved route (start to destination) and its local departure-to-expected-return time range. It uses the canonical `starting_location`, `destination`, `departure_at`, and `expected_return_at` request fields.
