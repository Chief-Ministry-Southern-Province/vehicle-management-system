...
- `/driverdetails` permits subject officers and executive roles and shows the Driver Directory as a horizontally scrollable table with selection, profile photo (initials fallback), name/ID, licence expiry, status, and View Details. Search, status filtering, and selected-driver PDF exports remain available. View Details opens the full identity, contact, licence, duty, and vehicle allocation profile in a native modal dialog supporting Close and Escape.
