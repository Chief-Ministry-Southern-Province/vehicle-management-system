...
- The shared map displays Sri Lanka's territorial outline, shades the surrounding area, constrains navigation to the Sri Lanka view, and accepts selectable points only inside the local boundary polygons. Keep the detailed Natural Earth mainland and island rings in order and free of self-intersections: an invalid ring cuts valid land out of the SVG mask and point-in-polygon check. The backend applies the exact same coordinates to request submission, route preview, and reverse-geocoding coordinates; the latitude/longitude bounding box alone is not sufficient.
