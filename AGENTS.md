...
- Administration: deputy secretaries and system administrators may use `POST /register` (the server sends the generated temporary password by SMS); `GET /users`; `PATCH|DELETE /users/{user}`; `GET|POST /departments`; `DELETE /departments/{department}`; `GET /system/database-backups` lists persistent backup audit records, while `POST /system/database-backups` creates and downloads a database backup.
