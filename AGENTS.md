...
| `system_admin` | Administration-only role: view the System Admin Dashboard; create, list, update, and remove users; manage users, departments, reusable journeys, and the global odometer-reading requirement; create database backups; and use the Administration Panel. This role cannot review, allocate, recommend, approve, or otherwise alter individual vehicle-request workflows. |
...
- the persisted global `odometer_readings_required` policy defaults to required. A System Administrator may make readings optional for all drivers when vehicle meters are unreliable. When required, starting a scheduled journey requires `start_odometer_km`; completing an ongoing or issue journey requires `end_odometer_km` and, for a legacy trip without a saved start, its missing start. When optional, either reading may be omitted. Any supplied reading remains a nonnegative kilometer value with up to two decimal places (maximum 99,999,999.99); a supplied ending reading cannot be below the available starting reading. A saved starting reading cannot be changed. `actual_distance_km` remains null unless both readings exist. State and readings are saved together under transaction locks;
...
- Fuel Management offers All Vehicles or one exact vehicle registration (All Vehicles selected by default). All Vehicles combines refill quantities, costs, ledger records, and summaries while preserving the other filters. Fleet consumption sums each vehicle using its own efficiency; if a contributing vehicle-month has no calculable consumption, that fleet month remains not recorded. Monthly Fuel Overview compares recorded fuel filled with calculated consumption on a shared liters axis by month, with no cost series. Consumption loads `/approved-journeys`, filters completed requests to the selected vehicle/year and ledger search/fuel-type filters, groups completion dates in Asia/Colombo, deduplicates shared actual mileage, and multiplies actual kilometers by the stored numeric fuel efficiency. Missing distance/efficiency remains null; months without completed journeys show zero. This follows the requested multiplication formula without efficiency-unit conversion, so km/l and other stored units are not normalized to liters. The four annual stat cards show fuel cost, filled liters, calculated consumed liters, and remaining liters (annual filled minus calculated consumed, preserving negative values). They use the selected year/vehicle and search/fuel-type filters, independently of the ledger month selection. Annual consumption sums available recorded monthly consumption values rather than discarding the entire year when a month is missing. If some months are missing, consumed and remaining cards identify the total as partial and show the missing-month count; all recorded months missing yields not recorded, while no completed journeys yields zero consumption. Remaining fuel subtracts that same available consumption total from annual filled fuel. Loading/errors are shown instead of interim zero totals. Consumption loading/errors affect the overview and consumed/remaining cards; Monthly Fuel Cost Overview plots cost (LKR) by month. Both use the selected vehicle/year and ledger filters, summing persisted `fuel_details.cost` and `fuel_details.capacity`; quantities represent recorded refills. The vehicle selection also scopes summaries and ledger/export records. Fuel Records Ledger has no selection checkboxes; PDF export automatically includes all displayed records matching the vehicle, year, month, search, and fuel-type filters. Clicking either chart filters the ledger to that month.
- Vehicle records include identity/specification, capacity, fuel configuration and efficiency, compliance expiries, insurance, assignment, status, images, and JSON-backed service/repair/fuel history. Subject officers can add or delete persisted vehicle images from the vehicle details editor; deletion is applied when the vehicle update is saved and removes both the database reference and stored public image file. Vehicle Directory displays the returned `image_url` beside each vehicle, falling back to its truck icon when no image is saved or the image cannot load. New registrations capture revenue-licence, insurance, and emission expiry dates; `registration_expiry` remains available for existing records but is not shown in Register Vehicle. Register Vehicle offers the standard vehicle types plus an **Add a new vehicle type** choice; its custom entry is submitted and persisted as the canonical `vehicle_type` (nonempty string up to 100 characters).
...
- Maintenance Records Ledger has no selection checkboxes. Service PDF export automatically includes all displayed records matching the search, service type, year, and optional month filters; export is disabled when no records match.
- Repair Records Ledger has no selection checkboxes. Repair PDF export automatically includes all displayed records matching the search, repair type, year, and optional month filters; export is disabled when no records match.
...
- `SystemSetting`: persisted global administrative policy. `odometer_readings_required` defaults to true when no record exists, records the System Administrator who last changed it, and controls whether driver meter inputs are required; it does not remove validation from any value supplied.
...
- Administration: deputy secretaries and system administrators may use `POST /register` (the server sends the generated temporary password by SMS); `GET /users`; `PATCH|DELETE /users/{user}`; `GET|POST /departments`; `DELETE /departments/{department}`; `GET /system/database-backups` lists persistent backup audit records, while `POST /system/database-backups` creates and downloads a database backup. Only active system administrators may use `GET|PATCH /system/odometer-settings` to read or change the persisted global `odometer_readings_required` policy; it defaults to `true`.
...
- `GET /approved-journeys` permits active subject officers, deputy secretaries, secretaries, and senior deputy secretaries. Executive read access supplies the Fuel Management consumption chart and annual consumed/remaining fuel cards; fleet write permissions remain subject-officer-only. The `/fuelmanagement` client route explicitly allows these same four roles.
- Driver operations: `GET /driver/dashboard-stats`, `/scheduled-journeys`, `/trip-history`, `/assigned-vehicle`; `PATCH /driver/journeys/{id}/status`; `POST /driver/issue-reports`. Scheduled-journey payloads include the persisted `odometer_readings_required` policy, saved start/end locations, coordinates, authoritative route distance/geometry, a derived `round_trip_distance_km` equal to twice the authoritative one-way distance, and per-request route data for consolidated journeys.
- Issue review: `GET /issue-reports`.
- Driver journey status actions accept `start_odometer_km` on start and `end_odometer_km` on completion (plus the missing start for legacy trips). They are mandatory only while the global odometer policy is required; supplied readings are always validated. Scheduled and successful status-action payloads include the current `odometer_readings_required` policy, while scheduled, status-action, and trip-history payloads expose both readings and `actual_distance_km`.
...
- System administrators alone may use `/odometersettings` to change the persisted global meter-reading requirement. It defaults to required. The driver schedule consumes the returned policy so its start and completion inputs are required only while the policy is enabled; an optional blank reading is not sent as a fabricated value. Status actions remain server-validated against the current policy and return it after a successful action, so a browser-held policy cannot weaken the workflow rule.
- Each role has a dashboard under `frontend/src/pages/dashboard/`.
- `/totalapprovals` explicitly restricts client access to deputy secretaries. Its Official Approval Records table has six columns: request number, requester, department, purpose/route/departure and return times, status, and View. The requester cell displays the saved profile image with initials fallback. View fetches the authorized approval-detail endpoint and opens a read-only native dialog with request/route/schedule/passenger/attachment details, recommendation and priority, current/previous allocation, approval and cancellation/rejection audit fields, and journey odometer readings. The dialog supports Close and Escape; the table scrolls horizontally on narrow screens.
- The administration-only `/usermanagement` page lets deputy secretaries and system administrators edit registered users' employee ID, name, email, phone, department, and account status. It displays roles read-only because driver role changes require the dedicated registration workflow. Saving a driver user's details synchronizes its linked driver directory record. `/departmentmanagement` separately creates, lists, and removes departments. System administrators use `/journeymanagement` to create, edit, and delete reusable journeys. `/databasemanagement` separately creates and downloads a complete database backup. Each completed backup saves a durable audit record with its filename, size, creator, database driver, and creation time, so Backup History remains visible after refresh; the backup data file is not retained or downloadable again from that history. Backup files are created in private server storage for the response and deleted after the download is sent. SQLite backups use a consistent database copy; MySQL and MariaDB backups use the configured `DATABASE_DUMP_BINARY` (default `mysqldump`) and add the configured password through the child process environment while preserving the server's existing environment. On Windows, the default automatically prefers an installed MySQL Server `mysqldump.exe` and uses XAMPP's bundled tool only when no MySQL Server tool is found.
- `/departmentrequesthistory` explicitly restricts client access to department officers. Its Request History header, filters, archive table, and View action preserve the department-scoped API records and existing query, status-filter, reset, refresh, pending-review navigation, and details navigation behavior. The Request Archive requester cell displays the saved profile image with initials fallback.
...
- Each received `vms:workflow-updated` Reverb invalidation immediately reloads the signed-in recipient's durable notification list, updating the bell badge, role-sidebar badges, open menu, and one-time in-app pop-up without a browser refresh. The backend API and `php artisan reverb:start` must both remain running, with matching public `VITE_REVERB_*` and backend `REVERB_*` app settings. `REVERB_ALLOWED_ORIGINS` accepts comma-separated hostnames or full SPA origins; configuration normalizes full origins to hostnames because Reverb compares against the hostname parsed from browser Origin headers.
...
- The deputy secretary's Pending Approvals table displays each requester’s saved profile image beside their name and employee ID. Its approval-list API eager-loads the canonical `user.profile_picture_path`; initials remain visible when no image exists or an image cannot load.
- The deputy secretary's Pending Allocation Queue additionally displays each request's abbreviated saved route (start to destination), local schedule date (both dates when an overnight journey spans days), and departure-to-expected-return time range. It uses the canonical `starting_location`, `destination`, `departure_at`, and `expected_return_at` request fields.
- The subject officer's Pending Journeys Recommended Requests table displays each requester's saved profile image beside their name and request ID, with initials fallback. Its recommended-requests API eager-loads the canonical `user.profile_picture_path`.
- The Approved Journeys table displays each requester's saved profile image beside their name and request ID, with initials fallback. Its approved-journeys API eager-loads the canonical `user.profile_picture_path`.
- When an eligible driver with an overlapping scheduled journey is selected in the deputy secretary's Vehicle Allocation section, it displays that scheduled journey's route, local time range, vehicle, passenger count, and purpose. The allocation-time driver response supplies these records in `scheduled_journeys`; they are derived from active overlapping requests and exclude completed journeys.
- The Pending Final Approvals table displays each requester’s saved profile image beside their name and employee ID. Its final-approval-list API eager-loads the canonical `user.profile_picture_path`; initials remain visible when no image exists or an image cannot load.
- The Department Officer dashboard's Pending Recommendations cards likewise display each department requester's saved profile image beside their name and employee ID, with the same initials fallback. Its department-scoped request API eager-loads `user.profile_picture_path`.
...
- Scheduled Journeys View opens a mobile-friendly details panel grouped into route/purpose/status, full departure and expected-return date-times, passengers, and vehicle/parking. Below the desktop breakpoint (1024px), the cards hide the requester/purpose/vehicle/parking summary and consolidated passenger pickup/drop-off rows; these remain available through View Details. Schedule, trip actions, saved starting meter readings, and reallocation notices remain visible on the cards. Secondary vehicle, compliance, and reallocation information is expandable. Planned one-way and round-trip distances are explicitly distinguished; consolidated requests retain their individual details.
...
- `/driverdetails` permits subject officers and executive roles and shows the Driver Directory as a horizontally scrollable table with profile photo (initials fallback), name/ID, licence expiry, status, and View Details. Search and status filtering remain available. Selection checkboxes are removed; PDF export automatically includes every driver matching the current filters, or the entire directory when no filters are applied. View Details opens the full identity, contact, licence, duty, and vehicle allocation profile in a native modal dialog supporting Close and Escape. Licence expiry badges in the table and profile are red for dates before today in Asia/Colombo and green for today or any future date; missing or invalid dates remain neutral.
- The Subject Officer and deputy secretary Fuel Analysis screen lists completed requests with request number, vehicle registration, driver name, start-to-destination route, allocated round-trip distance (`distance_km * 2`), actual distance, and a View more dialog with request, passenger, schedule, allocation, approval, attachment, and odometer details. The Journey Route column displays only the first comma-separated place name for each endpoint (for example, Matara - Galle); View more retains full addresses and missing labels retain coordinate fallbacks. Fuel Analysis filters combine case-insensitive partial vehicle-registration and driver-name matches with optional inclusive completion-date bounds using `journey_completed_at` in Asia/Colombo. Missing or invalid completion timestamps are excluded only when a date bound is set; reversed ranges show a validation message and no results. Clear filters resets all four inputs. The stat cards are replaced by two monthly bar charts: allocated round-trip versus actual distance (km), and Extra Fuel (L), with month/year on the horizontal axes. Both charts and table footer totals follow the filters. Charts group by completion month in Asia/Colombo, sort chronologically, exclude invalid/missing completion timestamps, and reuse the table total rules (including consolidated actual-distance deduplication and per-request Extra Fuel sums). Missing measurements remain null rather than zero. Allocated totals sum each request's planned round trip, including consolidated members. Actual totals count consolidated requests with the same vehicle, driver, start/completion timestamps, and meter readings once. Missing distances display as not recorded and are excluded; zero remains valid. Planned distance never substitutes for actual mileage. The Extra Fuel (L) footer totals the displayed requests' two-decimal Extra Fuel values, including negative values and consolidated member rows, skips missing values, and shows not recorded if none are available. The Extra Fuel (L) column uses the requested formula `(actual_distance_km - allocated round-trip distance) * allocated_vehicle.fuel_efficiency`, preserves negative results, and shows not recorded for missing distances or missing/invalid efficiency. It uses the stored numeric efficiency without unit conversion; vehicle efficiency units currently include km/l, l/100km, and km/kwh, so this requested formula is not a unit-normalized liters estimate.
...
- The shared map constrains navigation to the Sri Lanka view and accepts selectable points only inside the local boundary polygons. It intentionally does not render the territorial outline or a surrounding-area mask: map tiles and the saved road route must remain unobscured. Keep the detailed Natural Earth mainland and island rings in order and free of self-intersections because they support the point-in-polygon check. The backend applies the exact same coordinates to request submission, route preview, and reverse-geocoding coordinates; the latitude/longitude bounding box alone is not sufficient.
...
php artisan serve
# Keep this running in a second backend terminal for live notification updates.
...
`UserSeeder` creates or updates only the System Administrator from `SYSTEM_ADMIN_USERNAME`, `SYSTEM_ADMIN_NAME`, `SYSTEM_ADMIN_EMAIL`, `SYSTEM_ADMIN_PHONE`, `SYSTEM_ADMIN_DEPARTMENT`, and `SYSTEM_ADMIN_PASSWORD` through `config/system_admin.php`; the username is stored as `employee_id`. Use securely managed, environment-specific credentials in production. No vehicle, driver, or non-administrator demo data is seeded.
...
- Allocation-required SMS messages to deputy secretaries append the configured `FRONTEND_URL` plus `/approval/{request-id}`. Final-approval-required SMS messages to senior deputy secretaries and secretaries append `/final-approvals/{request-id}`. A recipient who is already signed in can open the respective protected SPA page directly from the link.
