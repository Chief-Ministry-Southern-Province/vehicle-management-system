...
- Fuel Management has an exact vehicle-registration selector (defaulting to the first registration in sorted order). Its Monthly Fuel Overview plots monthly recorded `fuel_details.capacity` quantities in liters for that vehicle and selected year, with liter tooltips rather than cost series. The same selection scopes the ledger, summaries, and exportable records; changing vehicles clears record selections. These quantities are recorded refills, not measured engine consumption.
