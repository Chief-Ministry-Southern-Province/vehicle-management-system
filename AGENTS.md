...
- The `Topbar` profile image opens `/setting` while preserving the current internal page as its return destination. Selecting the profile image again from User Settings returns to that preserved page; direct settings-page visits fall back to the signed-in role's dashboard.
